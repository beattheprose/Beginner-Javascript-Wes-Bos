...
The callback would be invoked four times, with the arguments and return values in each acall being as follows:
...
// TASK: make a tally of appearances for every letter and number in the text

// make an array of all letters
const charArray: string[] = text.split(``);

// remove the invalid characters from the array
const validArray = charArray.filter((letter: string) =>
  /[a-zA-Z\d]/.test(letter)
);

// make all case lowercase to aviod issues
const cleanArray = validArray.map((letter) => letter.toLowerCase());

// reduce to create an object with counts inside
const reducer = (acc: any, letter: string) => {
...
const countObj: CountInt = cleanArray.reduce(reducer, {});
console.table(countObj);

const sortByValue = (x: [string, number], y: [string, number]) => y[1] - x[1];

const popularityContest: any = Object.entries(countObj).sort(sortByValue);
console.table(popularityContest);